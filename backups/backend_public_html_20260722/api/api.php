...
function getDbConnection(): PDO {
    ...
    // --- Resolve credentials (env first, then built-in fallback for cPanel/LiteSpeed) ---
    $dbHost = (!empty($_SERVER['DB_HOST'])) ? $_SERVER['DB_HOST'] : ((!empty(getenv('DB_HOST'))) ? getenv('DB_HOST') : '198.251.83.217');
    ...
    // Resolve password (env first, then fallback for cPanel hosts where SetEnv is unavailable)
    $dbPass = '';
...
function initializeDefaultAdmin(PDO $pdo): void {
    try {
        ...
        if ($checkCount == 0) {
            ...
            $insertStmt->execute(['ADM001', 'admin', 'admin', 'Ismail Kallan']);
            api_log("Admin Initialization: Successfully seeded default credentials (admin/admin) for Ismail Kallan");
        }
    } catch (PDOException $e) {
...
function fetchAll(PDO $pdo): array {
    ...
    foreach ($tables as $tbl) {
        ...
        foreach ($rows as &$row) {
            ...
            if ($tbl === 'members' && empty($row['memberId']) && !empty($row['id'])) {
                $row['memberId'] = $row['id'];
            }
...
function upsertRow(PDO $pdo, string $table, array $body): array {
    ...
    foreach ($body as $col => &$val) {
        if (strpos(strtolower($col), 'amount') !== false || strpos(strtolower($col), 'fee') !== false) {
            $val = (float) $val;
        }
    }
