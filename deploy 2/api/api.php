...
// Force clean CORS handshake headers first
...
try {
    ...
    function fetchAll(PDO $pdo): array {
        ...
        foreach ($tables as $tbl) {
            try {
                ...
                echo json_encode([
                    'success' => false,
                    'error' => "Error fetching data from table: {$tbl}."
                ]);
                exit;
            }
    ...
    function upsertRow(PDO $pdo, string $table, array $body): array {
        ...
        try {
            ...
            echo json_encode([
                'success' => false,
                'error' => 'Database query failed during upsert check.'
            ]);
            exit;
        }
        ...
        if ($exists) {
            ...
            try {
                ...
                echo json_encode([
                    'success' => false,
                    'error' => 'Database query failed during update.'
                ]);
                exit;
            }
        } else {
            ...
            try {
                ...
                echo json_encode([
                    'success' => false,
                    'error' => 'Database query failed during insert.'
                ]);
                exit;
            }
        }
    ...
    function deleteRow(PDO $pdo, string $table, array $body): array {
        if (isset($body['action']) && $body['action'] === 'clear_all') {
            ...
            foreach ($tables as $tbl) {
                try {
                    ...
                    echo json_encode([
                        'success' => false,
                        'error' => "Failed to clear table: {$tbl}."
                    ]);
                    exit;
                }
            }
        ...
        if ($table === 'expenses' && isset($body['id'])) {
            try {
                ...
                echo json_encode([
                    'success' => false,
                    'error' => 'Database query failed during delete.'
                ]);
                exit;
            }
        }
    ...
    // Catch any syntax error, database offline, or missing files
    // Return a clean 200 OK with a JSON error so LiteSpeed NEVER crashes
    http_response_code(200); 
    echo json_encode([
        'success' => false,
        'error' => 'Backend system temporarily offline',
